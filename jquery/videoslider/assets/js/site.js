...
// });
	...
	var updateIpTags = function(thisEl) {
		...
		var timeadd = setTimeout(function() {
			thisSearch = $(thisEl).closest('.tab-col').find('.search-field .bootstrap-tagsinput.tagsInput .label-info');

			currentElem = $(thisEl).closest('.tab-col').find('.search-field .bootstrap-tagsinput.tagsInput').children('.label-info');
			...
			//console.log(JSON.stringify(thisSearch.text()))
			//$.makeArray(thisSearch).reverse()

			//console.log($('.tab-content .tab-1 .search-field .bootstrap-tagsinput.tagsInput .mCSB_container .label-info').length)

			if ($('.tab-content .tab-1 .search-field .bootstrap-tagsinput.tagsInput .mCSB_container .label-info').length == 0) {
				//console.log("refresh")
				$('tab-content .tab-col .search-field .input').tagsinput('refresh');
			}

			$('.tab-content .tab-col .search-field .bootstrap-tagsinput.tagsInput .mCSB_container input').before(currentElem);
	...
	$("body").on('mousedown', '.feature_section_list li', function() {
	...
	*/
	...
	$('body').on(clickEventglobal, '.home-slider .bx-pager.bx-default-pager a', function(event) {
         i = $(this).parent(".bx-pager-item").index();
	 	thiss.src(videosHref[$(this).parent(".bx-pager-item").index()]);
		 // i = ($(this).parent(".bx-pager-item").index()==len-1)?0:$(this).parent(".bx-pager-item").index()+1;
		
					

		// event.stopPropagation()
	})
	$('body').on(clickEventglobal, '.home-slider .bx-controls-direction .bx-prev', function(event) {
      i--;
      if(i<0){
      	i=len-1;
      }
		thiss.src(videosHref[i]);
					

		event.stopPropagation()
	})
	$('body').on(clickEventglobal, '.home-slider .bx-controls-direction .bx-next', function(event) {
      i++;
      if(i==len){
      	i=0;
      }
	...
	$('body').on('click', function() {
		...
		//console.log($('project_deatil_right.down-anim .agent-form-fixed h2'))
	})
	...
	var holydayStart = function() {
		...
		if (windowsTop + $(window).outerHeight() > hlistSaleOffset) {
			...
			$('.holyday-start .agent_form').slideUp(200);

		} else if (windowsTop + $(window).outerHeight() < hlistSaleOffset) {

			$('.holyday-start .agent_form').slideDown(200);
...
$(function() {
	...
	$('body').on('click', '.tab-row a', function() {
		$('.mCSB_scrollTools .mCSB_dragger .mCSB_dragger_bar').hide();
		var linet = setTimeout(function() {
			$('.mCSB_scrollTools .mCSB_dragger .mCSB_dragger_bar').show();
		}, 55)
	})
	//
})